#!/usr/bin/env node
/**
 * enrich-plays-b8.js — Batch B8: Enrich solution plays 71-80
 * Run: node scripts/enrich-plays-b8.js
 */
const fs = require("fs");
...
// Play metadata — B8 (plays 71-80)
const playMeta = {
  "71-smart-energy-grid-ai": {
    name: "Smart Energy Grid AI", pattern: "Energy Digital Twin", model: "gpt-4o",
    services: ["Azure IoT Hub", "Azure OpenAI", "Azure Stream Analytics", "Azure Digital Twins", "Azure Functions", "Azure Cosmos DB", "Azure Monitor"],
    ...
    domain: "energy demand prediction, renewable source optimization (solar/wind/hydro), grid balancing via digital twin simulation, battery storage scheduling, peak load management, outage prediction, carbon-aware load shifting",
    tuning: "demand_prediction_horizon_hours (1-168), renewable_mix_targets, battery_schedule_optimization, stability_margins (frequency/voltage), peak_load_threshold_mw, carbon_intensity_source, grid_simulation_interval_ms",
  },
  "72-climate-risk-assessor": {
    name: "Climate Risk Assessor", pattern: "Scenario Modeling", model: "gpt-4o",
    services: ["Azure OpenAI", "Azure Machine Learning", "Azure Cosmos DB", "Azure AI Search", "Azure Functions", "Azure Monitor"],
    ...
    domain: "climate scenario modeling (RCP 2.6/4.5/8.5, SSP1-5), physical risk assessment (flood/drought/wildfire/sea-level), transition risk scoring (carbon pricing/regulation), liability risk estimation, financial impact quantification, TCFD-aligned reporting",
    tuning: "scenarios_rcp (2.6/4.5/8.5), time_horizons_years (2030/2050/2100), risk_tolerance (conservative/moderate/aggressive), geography_granularity (country/region/asset), financial_impact_model, tcfd_reporting_enabled",
  },
  "73-waste-recycling-optimizer": {
    name: "Waste & Recycling Optimizer", pattern: "AI Waste Management", model: "gpt-4o",
    services: ["Azure AI Vision", "Azure OpenAI", "Azure IoT Hub", "Azure Container Apps", "Azure Cosmos DB", "Azure Monitor"],
    waf: ["reliability", "cost-optimization", "performance-efficiency", "security"],
    knowledge: ["F1-GenAI-Foundations", "T3-Production-Patterns", "O2-Agent-Coding"],
    domain: "material classification via computer vision (plastic/paper/metal/glass/organic), route optimization for collection vehicles, contamination detection, fill-level monitoring via IoT sensors, recycling rate tracking, regulatory compliance reporting",
    tuning: "material_categories (10-50 classes), classification_confidence_threshold (0.85), vehicle_capacity_kg, route_optimization_algorithm (TSP/VRP), fill_level_alert_percentage (80%), contamination_severity_levels",
  },
  "74-ai-tutoring-agent": {
    name: "AI Tutoring Agent", pattern: "Socratic Learning", model: "gpt-4o",
    services: ["Azure OpenAI", "Azure Cosmos DB", "Azure AI Search", "Azure Static Web Apps", "Azure Functions", "Azure Monitor"],
    ...
    domain: "1-on-1 personalized tutoring, Socratic method (questions over answers), knowledge gap detection via adaptive assessment, difficulty auto-adjustment, progress tracking, multi-subject support, learning style adaptation (visual/auditory/kinesthetic)",
    tuning: "socratic_depth (1-5 probing questions), difficulty_adaptation_speed (slow/medium/fast), gap_detection_threshold (score<0.6), subject_domains, session_length_minutes (15-60), learning_style_detection_enabled, encouragement_frequency",
  },
  "75-exam-generation-engine": {
    name: "Exam Generation Engine", pattern: "Assessment Automation", model: "gpt-4o",
    services: ["Azure OpenAI", "Azure Blob Storage", "Azure Cosmos DB", "Azure Functions", "Azure Key Vault", "Azure Monitor"],
    waf: ["security", "reliability", "responsible-ai", "cost-optimization"],
    knowledge: ["R1-Prompt-Engineering", "T2-Responsible-AI", "T3-Production-Patterns"],
    domain: "auto-generate exams from curriculum documents, difficulty calibration using Bloom's taxonomy, automatic rubric generation, anti-cheating question variation, distractor quality validation, item response theory analysis, accessibility compliance (WCAG)",
    tuning: "difficulty_distribution (easy:30%/medium:50%/hard:20%), blooms_levels (remember/understand/apply/analyze/evaluate/create), variation_count_per_question (3-5), distractor_quality_threshold, item_bank_size, wcag_compliance_level (AA/AAA)",
  },
  "76-accessibility-learning-agent": {
    name: "Accessibility Learning Agent", pattern: "Inclusive Education", model: "gpt-4o",
    services: ["Azure AI Speech", "Azure OpenAI", "Azure AI Vision", "Azure Container Apps", "Azure Cosmos DB", "Azure Monitor"],
    waf: ["responsible-ai", "reliability", "security", "performance-efficiency"],
    knowledge: ["T2-Responsible-AI", "O2-Agent-Coding", "T3-Production-Patterns"],
    domain: "screen reader-first content design, dyslexia-aware text rendering (OpenDyslexic font/spacing), multi-modal content adaptation (text-to-speech/speech-to-text/image descriptions), cognitive load management, WCAG 2.1 AA compliance, personalized accessibility profiles",
    tuning: "wcag_level (A/AA/AAA), dyslexia_settings (font_size/line_spacing/word_spacing), speech_rate (0.5-2.0x), content_simplification_level (original/simplified/easy_read), alt_text_generation_enabled, cognitive_load_max_items (3-7)",
  },
  "77-research-paper-ai": {
    name: "Research Paper AI", pattern: "Academic Intelligence", model: "gpt-4o",
    services: ["Azure OpenAI", "Azure AI Search", "Azure Cosmos DB", "Microsoft Graph", "Azure Functions", "Azure Monitor"],
    waf: ["security", "reliability", "responsible-ai", "cost-optimization"],
    knowledge: ["R2-RAG-Architecture", "R1-Prompt-Engineering", "T3-Production-Patterns"],
    domain: "literature review automation, citation network analysis, methodology critique and scoring, research gap identification, abstract/summary generation, plagiarism-aware paraphrasing, domain-specific terminology management, peer review assistance",
    tuning: "citation_depth (1-3 hops in citation graph), methodology_rigor_scoring (qualitative/quantitative/mixed), domain_specialization (STEM/social/humanities), gap_identification_sensitivity, summary_style (abstract/executive/detailed), plagiarism_check_enabled",
  },
  "78-precision-agriculture-agent": {
    name: "Precision Agriculture Agent", pattern: "Smart Farming", model: "gpt-4o",
    services: ["Azure IoT Hub", "Azure AI Vision", "Azure OpenAI", "Azure Digital Twins", "Azure Machine Learning", "Azure Monitor"],
    waf: ["reliability", "performance-efficiency", "cost-optimization", "security", "operational-excellence"],
    knowledge: ["O5-GPU-Infra", "T1-Fine-Tuning-MLOps", "T3-Production-Patterns"],
    domain: "satellite imagery analysis for crop health (NDVI), IoT soil sensor fusion (moisture/pH/nitrogen), irrigation scheduling optimization, fertilization recommendations, yield prediction models, pest/disease early detection, weather integration",
    tuning: "sensor_sampling_interval_minutes (5-60), imagery_frequency_days (1-7), irrigation_threshold_soil_moisture (30-60%), ndvi_health_threshold, pest_detection_confidence (0.8), weather_forecast_source, yield_prediction_horizon_weeks",
  },
  "79-food-safety-inspector-ai": {
    name: "Food Safety Inspector AI", pattern: "HACCP Compliance", model: "gpt-4o",
    ...
    tuning: "temperature_alert_thresholds_celsius (per_product_type), pathogen_models (salmonella/listeria/ecoli), audit_retention_years (5), traceability_depth (farm/processor/distributor/retailer), recall_trigger_rules, haccp_critical_control_points",
  },
  "80-biodiversity-monitor": {
    name: "Biodiversity Monitor", pattern: "Conservation Intelligence", model: "gpt-4o",
    services: ["Azure AI Vision", "Azure OpenAI", "Azure IoT Hub", "Azure Cosmos DB", "Azure Functions", "Azure Monitor"],
    waf: ["reliability", "responsible-ai", "cost-optimization", "security"],
    knowledge: ["F1-GenAI-Foundations", "T2-Responsible-AI", "T3-Production-Patterns"],
    domain: "species identification from camera trap images, drone aerial survey analysis, acoustic monitoring (bird/bat/marine), biodiversity index calculation (Shannon/Simpson), conservation alert generation, habitat change detection, poaching activity detection",
    tuning: "species_confidence_threshold (0.85), camera_schedule (continuous/motion-triggered), acoustic_frequency_range_hz, biodiversity_index_method (Shannon/Simpson), habitat_change_detection_interval_days, alert_severity_levels, poaching_detection_enabled",
  },
};
