#!/usr/bin/env node
/**
 * enrich-plays-b3.js — Batch B3: Enrich solution plays 21-30
 * Run: node scripts/enrich-plays-b3.js
 */
const fs = require("fs");
...
// Play metadata for enrichment — B3 (plays 21-30)
const playMeta = {
  "21-agentic-rag": {
    name: "Agentic RAG", pattern: "Autonomous Retrieval Agent", model: "gpt-4o",
    services: ["Azure OpenAI", "Azure AI Search", "Azure Container Apps", "Azure Key Vault", "Azure Cosmos DB", "Azure Monitor"],
    waf: ["security", "reliability", "cost-optimization", "performance-efficiency", "responsible-ai"],
    knowledge: ["R2-RAG-Architecture", "O2-Agent-Coding", "O3-MCP-Tools-Functions", "T3-Production-Patterns"],
    domain: "autonomous retrieval, agent-controlled search, multi-source fusion, iterative refinement, citation generation, source ranking, query decomposition, self-reflection",
    tuning: "retrieval_strategy (single/multi/iterative), source_ranking_weights, iteration_depth (1-5), citation_format, query_decomposition_enabled, reflection_threshold, max_sources",
  },
  "22-multi-agent-swarm": {
    name: "Multi-Agent Swarm", pattern: "Distributed Agent Teams", model: "gpt-4o",
    services: ["Azure OpenAI", "Azure Container Apps", "Azure Service Bus", "Azure Cosmos DB", "Azure Monitor", "Azure Key Vault"],
    waf: ["security", "reliability", "cost-optimization", "operational-excellence", "performance-efficiency"],
    knowledge: ["O2-Agent-Coding", "O3-MCP-Tools-Functions", "T3-Production-Patterns"],
    domain: "swarm topology (mesh/star/hierarchical), supervisor pattern, tool delegation, shared memory via Cosmos DB, conflict resolution, consensus voting, agent specialization, loop detection",
    tuning: "team_topology, delegation_rules, memory_config (TTL/max_size), conflict_policy (majority/supervisor), max_iterations=10, agent_count, consensus_threshold",
  },
  "23-browser-automation-agent": {
    name: "Browser Automation", pattern: "AI Web Task Execution", model: "gpt-4o",
    services: ["Azure OpenAI (GPT-4o Vision)", "Azure Container Apps", "Playwright MCP", "Azure Key Vault", "Azure Monitor"],
    waf: ["security", "reliability", "cost-optimization"],
    knowledge: ["O2-Agent-Coding", "O3-MCP-Tools-Functions", "F1-GenAI-Foundations"],
    domain: "Playwright browser automation, GPT-4o Vision for page understanding, form filling, navigation, data extraction, screenshot analysis, cookie/session management, headless Chrome",
    tuning: "domain_allowlist, vision_prompt_templates, action_timeout_ms (5000-30000), retry_config, max_steps_per_task, screenshot_resolution, viewport_size",
  },
  "24-ai-code-review-pipeline": {
    name: "AI Code Review", pattern: "Automated PR Review", model: "gpt-4o",
    services: ["Azure OpenAI", "GitHub Actions", "CodeQL", "Azure DevOps", "Azure Monitor"],
    waf: ["security", "operational-excellence", "reliability"],
    knowledge: ["O2-Agent-Coding", "T3-Production-Patterns", "F4-GitHub-Agentic-OS"],
    domain: "PR review automation, CodeQL security scanning, OWASP vulnerability detection, inline AI comments, architecture validation, style enforcement, dependency audit, test coverage analysis",
    tuning: "severity_thresholds (critical/high/medium/low), review_depth (quick/standard/deep), owasp_rules, style_checks, min_test_coverage (80%), max_file_size_for_review",
  },
  "25-conversation-memory-layer": {
    name: "Conversation Memory", pattern: "Persistent Agent Memory", model: "gpt-4o",
    ...
    domain: "short-term memory (Redis), long-term memory (Cosmos DB), episodic memory (AI Search vectors), cross-session learning, memory consolidation, forgetting policies, semantic recall",
    tuning: "memory_tiers (short/long/episodic), retention_policy_days, recall_strategy (semantic/temporal/hybrid), embedding_model, max_memory_items, consolidation_schedule, redis_ttl_seconds",
  },
  "26-semantic-search-engine": {
    name: "Semantic Search Engine", pattern: "Hybrid Vector Search", model: "gpt-4o",
    services: ["Azure AI Search", "Azure OpenAI", "Azure Blob Storage", "Azure Container Apps", "Azure Monitor"],
    waf: ["security", "reliability", "performance-efficiency", "cost-optimization"],
    knowledge: ["R2-RAG-Architecture", "T3-Production-Patterns", "F1-GenAI-Foundations"],
    domain: "full-text + vector hybrid search, semantic reranking, personalization, query expansion, auto-suggest, faceted filtering, answer generation with citations, relevance feedback loop",
    tuning: "hybrid_weights (keyword:0.4/vector:0.6), reranker_model, personalization_enabled, result_count (10-50), relevance_threshold (0.65-0.85), answer_generation_enabled, embedding_model",
  },
  "27-ai-data-pipeline": {
    name: "AI Data Pipeline", pattern: "LLM-Augmented ETL", model: "gpt-4o-mini",
    services: ["Azure OpenAI", "Azure Data Factory", "Azure Blob Storage", "Azure Event Hubs", "Azure Cosmos DB", "Azure Monitor"],
    ...
    domain: "ETL with LLM augmentation, data classification, PII detection and redaction, schema inference, quality scoring, enrichment pipelines, batch vs streaming, data lineage tracking",
    tuning: "classification_prompts, pii_rules (detect/redact/mask), quality_thresholds (0.8-0.95), batch_size (100-10000), enrichment_model (gpt-4o-mini for cost), retry_policy, parallelism_degree",
  },
  "28-knowledge-graph-rag": {
    name: "Knowledge Graph RAG", pattern: "Graph-Enhanced Retrieval", model: "gpt-4o",
    services: ["Azure OpenAI", "Azure Cosmos DB (Gremlin API)", "Azure AI Search", "Azure Container Apps", "Azure Monitor"],
    ...
    domain: "entity extraction (NER), relationship mapping, graph traversal for context, Gremlin queries, hybrid graph+vector retrieval, explainable answers with provenance chains, ontology management",
    tuning: "graph_depth (1-4 hops), entity_types, relationship_weights, answer_style (concise/detailed/explainable), entity_confidence_threshold, max_graph_nodes_per_query, embedding_model",
  },
  "29-mcp-gateway": {
    name: "MCP Gateway", pattern: "Tool Proxy & Management", model: "gpt-4o-mini",
    services: ["Azure API Management", "Azure Container Apps", "Azure Monitor", "Azure Key Vault", "Azure Redis Cache", "Azure Cosmos DB"],
    waf: ["security", "reliability", "cost-optimization", "operational-excellence"],
    knowledge: ["O3-MCP-Tools-Functions", "T3-Production-Patterns", "F4-GitHub-Agentic-OS"],
    domain: "MCP protocol proxy, tool discovery registry, rate limiting per tool/user, OAuth2/API key auth, usage analytics, tool versioning, circuit breaker for failing tools, audit logging",
    tuning: "rate_limits_per_tool, auth_rules (oauth2/apikey/managed_identity), tool_registry_refresh_interval, analytics_retention_days, circuit_breaker_threshold, cache_ttl_seconds, max_concurrent_tool_calls",
  },
  "30-ai-security-hardening": {
    name: "AI Security Hardening", pattern: "LLM Security Defense", model: "gpt-4o",
    services: ["Azure AI Content Safety", "Azure OpenAI", "Azure Container Apps", "Azure Key Vault", "Azure Monitor", "Azure Policy"],
    waf: ["security", "reliability", "responsible-ai"],
    knowledge: ["T2-Responsible-AI", "T3-Production-Patterns", "R3-Deterministic-AI"],
    domain: "prompt injection detection (direct/indirect), jailbreak defense, OWASP LLM Top 10, red teaming automation, PII leakage prevention, model endpoint protection, safety scorecards, EU AI Act compliance",
    tuning: "injection_detection_sensitivity (low/medium/high), jailbreak_patterns, blocklist_terms, severity_thresholds (0-4), red_team_scenarios_count, pii_categories, compliance_framework (EU_AI_Act/NIST/ISO)",
  },
};
