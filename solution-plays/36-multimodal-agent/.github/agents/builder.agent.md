...
### Architecture Pattern: Vision+Text+Code Analysis
image analysis, screenshot understanding, diagram interpretation, code-from-image extraction, multi-modal routing (text vs vision vs code), document layout analysis, chart data extraction
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "36-multimodal-agent-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Multimodal Agent
vision_prompts, image_resolution (low/high/auto), multi_modal_routing_rules, max_image_size_mb, code_extraction_language, chart_data_output_format, vision_model_version
