---
name: "deploy-azure"
description: "Deploy Multimodal Agent (Play 36) infrastructure and application to Azure"
---

# Deploy Azure — Multimodal Agent

## Overview
This skill deploys the Multimodal Agent solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=36
```
...
az deployment group create \
  ...
  --name "deploy-play-36-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-36* --query properties.outputs -o json)
