---
name: "deploy-azure"
description: "Deploy Computer Use Agent (Play 42) infrastructure and application to Azure"
---

# Deploy Azure — Computer Use Agent

## Overview
This skill deploys the Computer Use Agent solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=42
```
...
az deployment group create \
  ...
  --name "deploy-play-42-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-42* --query properties.outputs -o json)
