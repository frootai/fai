---
name: "deploy-azure"
description: "Deploy Biodiversity Monitor (Play 80) infrastructure and application to Azure"
---

# Deploy Azure — Biodiversity Monitor

## Overview
This skill deploys the Biodiversity Monitor solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=80
```
...
az deployment group create \
  ...
  --name "deploy-play-80-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-80* --query properties.outputs -o json)
