...
### Architecture Pattern: Conservation Intelligence
species identification from camera trap images, drone aerial survey analysis, acoustic monitoring (bird/bat/marine), biodiversity index calculation (Shannon/Simpson), conservation alert generation, habitat change detection, poaching activity detection
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "80-biodiversity-monitor-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Biodiversity Monitor
species_confidence_threshold (0.85), camera_schedule (continuous/motion-triggered), acoustic_frequency_range_hz, biodiversity_index_method (Shannon/Simpson), habitat_change_detection_interval_days, alert_severity_levels, poaching_detection_enabled
