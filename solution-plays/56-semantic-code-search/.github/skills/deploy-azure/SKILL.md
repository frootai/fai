---
name: "deploy-azure"
description: "Deploy Semantic Code Search (Play 56) infrastructure and application to Azure"
---

# Deploy Azure — Semantic Code Search

## Overview
This skill deploys the Semantic Code Search solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=56
```
...
az deployment group create \
  ...
  --name "deploy-play-56-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-56* --query properties.outputs -o json)
