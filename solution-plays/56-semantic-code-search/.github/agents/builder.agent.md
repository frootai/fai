...
### Architecture Pattern: Natural Language Code Discovery
natural language to code search, function discovery by intent description, cross-repo navigation, dependency mapping, code explanation generation, API usage examples, semantic similarity across languages
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "56-semantic-code-search-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Semantic Code Search
embedding_model (text-embedding-3-large), index_config (chunk_by_function/class/file), relevance_threshold (0.70-0.90), supported_languages, max_results (10-50), code_explanation_enabled
