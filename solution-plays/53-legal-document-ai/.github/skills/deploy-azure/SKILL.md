---
name: "deploy-azure"
description: "Deploy Legal Document Ai (Play 53) infrastructure and application to Azure"
---

# Deploy Azure — Legal Document Ai

## Overview
This skill deploys the Legal Document Ai solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=53
```
...
az deployment group create \
  ...
  --name "deploy-play-53-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-53* --query properties.outputs -o json)
