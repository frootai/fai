...
- **Azure CDN** — use latest SDK, Managed Identity auth, private endpoints where available
- **Azure Media Services** — use latest SDK, Managed Identity auth, private endpoints where available

### Architecture Pattern: Text-to-Video Pipeline
text-to-video generation, prompt-driven scene composition, content safety pre-screening, batch processing pipeline, video resolution scaling, watermarking, format transcoding, CDN distribution
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "43-ai-video-generation-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for AI Video Generation
video_resolution (720p/1080p/4K), safety_thresholds, batch_size (1-10), generation_model, max_duration_seconds (15-60), watermark_enabled, output_format (mp4/webm), cost_limit_per_generation
