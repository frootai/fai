---
name: "deploy-azure"
description: "Deploy Ai Video Generation (Play 43) infrastructure and application to Azure"
---

# Deploy Azure — Ai Video Generation

## Overview
This skill deploys the Ai Video Generation solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=43
```
...
az deployment group create \
  ...
  --name "deploy-play-43-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-43* --query properties.outputs -o json)
