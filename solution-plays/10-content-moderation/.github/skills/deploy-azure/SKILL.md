---
name: "deploy-azure"
description: "Deploy Content Moderation (Play 10) infrastructure and application to Azure"
---

# Deploy Azure — Content Moderation

## Overview
This skill deploys the Content Moderation solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=10
```
...
az deployment group create \
  ...
  --name "deploy-play-10-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-10* --query properties.outputs -o json)
