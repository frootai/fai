# AI-Powered Deployment Workflow — Ai Sales Assistant

> Layer 3 — Agentic Workflow. Compiles to GitHub Actions for automated Azure deployment.

## Trigger
On push to `main` branch after PR merge, when files in `solution-plays/64-ai-sales-assistant/infra/` are modified.
...
```yaml
name: AI Deploy — Ai Sales Assistant
on:
  push:
    branches: [main]
    paths: ['solution-plays/64-ai-sales-assistant/infra/**']
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Bicep
        run: az bicep build --file solution-plays/64-ai-sales-assistant/infra/main.bicep
      - name: Validate Configs
        run: |
          for f in solution-plays/64-ai-sales-assistant/config/*.json; do
      ...
      - name: Deploy to Staging
        run: |
          az deployment group create \
            --resource-group rg-ai-sales-assistant-staging \
            --template-file solution-plays/64-ai-sales-assistant/infra/main.bicep \
            --parameters solution-plays/64-ai-sales-assistant/infra/parameters.json
  ...
  evaluate:
    ...
    steps:
      ...
      - name: Run Evaluation
        run: python solution-plays/64-ai-sales-assistant/evaluation/eval.py
        env:
  ...
  deploy-production:
    ...
    steps:
      ...
      - name: Deploy to Production
        run: |
          az deployment group create \
            --resource-group rg-ai-sales-assistant \
            --template-file solution-plays/64-ai-sales-assistant/infra/main.bicep \
            --parameters solution-plays/64-ai-sales-assistant/infra/parameters.json \
