---
name: "deploy-azure"
description: "Deploy Prompt Management (Play 18) infrastructure and application to Azure"
---

# Deploy Azure — Prompt Management

## Overview
This skill deploys the Prompt Management solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=18
```
...
az deployment group create \
  ...
  --name "deploy-play-18-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-18* --query properties.outputs -o json)
