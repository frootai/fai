...
- **Azure Prompt Flow** — use latest SDK, Managed Identity auth, private endpoints where available
- **Git** — use latest SDK, Managed Identity auth, private endpoints where available
...
- **Azure App Service** — use latest SDK, Managed Identity auth, private endpoints where available

### Architecture Pattern: Prompt Versioning & A/B
prompt versioning with semantic versioning, A/B testing with traffic splitting, rollback procedures, prompt templates with variable injection, few-shot example management, chain-of-thought variants, evaluation pipelines, environment promotion (dev→staging→prod)
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "18-prompt-management-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Prompt Management
prompt_versions (semver), ab_weights (90/10 default), rollback_rules, template_variables, few_shot_count (2-5), evaluation_metrics (quality/cost/latency), promotion_gates (all metrics above threshold)
