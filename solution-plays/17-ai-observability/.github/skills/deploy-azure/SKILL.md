---
name: "deploy-azure"
description: "Deploy Ai Observability (Play 17) infrastructure and application to Azure"
---

# Deploy Azure — Ai Observability

## Overview
This skill deploys the Ai Observability solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=17
```
...
az deployment group create \
  ...
  --name "deploy-play-17-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-17* --query properties.outputs -o json)
