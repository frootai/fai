...
- **Azure Application Insights** — use latest SDK, Managed Identity auth, private endpoints where available
- **Azure Log Analytics** — use latest SDK, Managed Identity auth, private endpoints where available
- **Azure Monitor** — use latest SDK, Managed Identity auth, private endpoints where available
- **Azure Workbooks** — use latest SDK, Managed Identity auth, private endpoints where available
- **Azure Alerts** — use latest SDK, Managed Identity auth, private endpoints where available
- **KQL** — use latest SDK, Managed Identity auth, private endpoints where available

### Architecture Pattern: Monitoring & Alerting
KQL query authoring, custom AI metrics (groundedness/coherence/latency/token_usage/cost_per_query), smart alerting rules, workbook dashboards, distributed tracing with correlation IDs, SLO/SLI tracking
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "17-ai-observability-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for AI Observability
kql_queries, alert_thresholds (latency_p95/error_rate/quality_score), quality_metrics, sampling_rate (0.1-1.0), retention_days (30-90), dashboard_refresh_interval, anomaly_detection_sensitivity
