---
name: "deploy-azure"
description: "Deploy Network Optimization Agent (Play 90) infrastructure and application to Azure"
---

# Deploy Azure — Network Optimization Agent

## Overview
This skill deploys the Network Optimization Agent solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=90
```
...
az deployment group create \
  ...
  --name "deploy-play-90-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-90* --query properties.outputs -o json)
