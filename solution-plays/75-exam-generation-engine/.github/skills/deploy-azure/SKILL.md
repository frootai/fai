---
name: "deploy-azure"
description: "Deploy Exam Generation Engine (Play 75) infrastructure and application to Azure"
---

# Deploy Azure — Exam Generation Engine

## Overview
This skill deploys the Exam Generation Engine solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=75
```
...
az deployment group create \
  ...
  --name "deploy-play-75-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-75* --query properties.outputs -o json)
