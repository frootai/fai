---
name: "deploy-azure"
description: "Deploy Continual Learning Agent (Play 93) infrastructure and application to Azure"
---

# Deploy Azure — Continual Learning Agent

## Overview
This skill deploys the Continual Learning Agent solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=93
```
...
az deployment group create \
  ...
  --name "deploy-play-93-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-93* --query properties.outputs -o json)
