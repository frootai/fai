---
name: "deploy-azure"
description: "Deploy Waste Recycling Optimizer (Play 73) infrastructure and application to Azure"
---

# Deploy Azure — Waste Recycling Optimizer

## Overview
This skill deploys the Waste Recycling Optimizer solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=73
```
...
az deployment group create \
  ...
  --name "deploy-play-73-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-73* --query properties.outputs -o json)
