---
name: "deploy-azure"
description: "Deploy Ai Compliance Engine (Play 35) infrastructure and application to Azure"
---

# Deploy Azure — Ai Compliance Engine

## Overview
This skill deploys the Ai Compliance Engine solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=35
```
...
az deployment group create \
  ...
  --name "deploy-play-35-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-35* --query properties.outputs -o json)
