...
- **Azure GPU Compute** — use latest SDK, Managed Identity auth, private endpoints where available
- **Azure Blob Storage** — use latest SDK, Managed Identity auth, private endpoints where available
- **MLflow** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: LoRA Training Pipeline
LoRA/QLoRA fine-tuning, JSONL data preparation, hyperparameter search, evaluation metrics (perplexity, BLEU, ROUGE), model registry, deployment pipelines, experiment tracking, compute instance management
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "13-fine-tuning-workflow-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Fine-Tuning Workflow
LoRA_rank (8-64), learning_rate (1e-5 to 5e-4), num_epochs (1-5), batch_size, warmup_steps, weight_decay, eval_strategy, dataset_split_ratio, gradient_accumulation_steps
