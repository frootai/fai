---
name: "deploy-azure"
description: "Deploy Fine Tuning Workflow (Play 13) infrastructure and application to Azure"
---

# Deploy Azure — Fine Tuning Workflow

## Overview
This skill deploys the Fine Tuning Workflow solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=13
```
...
az deployment group create \
  ...
  --name "deploy-play-13-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-13* --query properties.outputs -o json)
