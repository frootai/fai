...
### Architecture Pattern: Zero-Temperature Agent
zero-temperature inference, seed pinning, structured JSON output, anti-sycophancy, confidence scoring, multi-layer guardrails
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "03-deterministic-agent-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Deterministic Agent
temperature=0, seed=42, JSON schema validation, confidence_threshold=0.7, max_retries=3
