---
name: "deploy-azure"
description: "Deploy Deterministic Agent (Play 03) infrastructure and application to Azure"
---

# Deploy Azure — Deterministic Agent

## Overview
This skill deploys the Deterministic Agent solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=03
```
...
az deployment group create \
  ...
  --name "deploy-play-03-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-03* --query properties.outputs -o json)
