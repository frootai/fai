# AI-Powered Code Review Workflow — Esg Compliance Agent

> Layer 3 — Agentic Workflow. Runs automated review on PRs targeting this play.

## Trigger
On pull request to `main` when files in `solution-plays/70-esg-compliance-agent/` are modified.
...
```yaml
name: AI Review — Esg Compliance Agent
on:
  pull_request:
    paths: ['solution-plays/70-esg-compliance-agent/**']
  ...
  review:
    ...
    steps:
      ...
      - name: Check JSON validity
        run: |
          for f in $(find solution-plays/70-esg-compliance-agent -name "*.json" -type f); do
            python3 -c "import json; json.load(open('$f'))" || echo "INVALID: $f"
          done

      - name: Check for secrets
        run: |
          if grep -rl "sk-[a-zA-Z0-9]\{20,\}" solution-plays/70-esg-compliance-agent/ 2>/dev/null; then
      ...
      - name: Check guardrails
        run: |
          python3 -c "
          import json
          g = json.load(open('solution-plays/70-esg-compliance-agent/config/guardrails.json'))
