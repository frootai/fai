---
name: "deploy-azure"
description: "Deploy Esg Compliance Agent (Play 70) infrastructure and application to Azure"
---

# Deploy Azure — Esg Compliance Agent

## Overview
This skill deploys the Esg Compliance Agent solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=70
```
...
az deployment group create \
  ...
  --name "deploy-play-70-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-70* --query properties.outputs -o json)
