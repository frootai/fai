---
name: "deploy-azure"
description: "Deploy Public Safety Analytics (Play 86) infrastructure and application to Azure"
---

# Deploy Azure — Public Safety Analytics

## Overview
This skill deploys the Public Safety Analytics solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=86
```
...
az deployment group create \
  ...
  --name "deploy-play-86-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-86* --query properties.outputs -o json)
