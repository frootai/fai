...
- **Azure AI Document Intelligence** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: Advanced Document Processing
multi-page PDF processing, cross-page table relationships, handwriting recognition, entity linking across documents, invoice/receipt parsing, contract clause extraction, layout analysis
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "38-document-understanding-v2-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Document Understanding v2
extraction_schemas, confidence_thresholds (0.7-0.95), entity_types, page_batch_size, cross_reference_enabled, handwriting_model_version, output_format (JSON/CSV)
