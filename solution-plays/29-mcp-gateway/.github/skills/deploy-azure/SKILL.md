---
name: "deploy-azure"
description: "Deploy Mcp Gateway (Play 29) infrastructure and application to Azure"
---

# Deploy Azure — Mcp Gateway

## Overview
This skill deploys the Mcp Gateway solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=29
```
...
az deployment group create \
  ...
  --name "deploy-play-29-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-29* --query properties.outputs -o json)
