...
### Architecture Pattern: Tool Proxy & Management
MCP protocol proxy, tool discovery registry, rate limiting per tool/user, OAuth2/API key auth, usage analytics, tool versioning, circuit breaker for failing tools, audit logging
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "29-mcp-gateway-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for MCP Gateway
rate_limits_per_tool, auth_rules (oauth2/apikey/managed_identity), tool_registry_refresh_interval, analytics_retention_days, circuit_breaker_threshold, cache_ttl_seconds, max_concurrent_tool_calls
