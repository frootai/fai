...
- **Azure Kubernetes Service** — use latest SDK, Managed Identity auth, private endpoints where available
- **NVIDIA GPU Nodes (A100/H100)** — use latest SDK, Managed Identity auth, private endpoints where available
- **vLLM** — use latest SDK, Managed Identity auth, private endpoints where available
- **Azure Container Registry** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: GPU Cluster Inference
GPU cluster management, model quantization (GPTQ/AWQ/INT4), vLLM serving engine, auto-scaling based on GPU utilization and request queue depth, model versioning, A/B canary deployment, readiness/liveness probes
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "12-model-serving-aks-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Model Serving AKS
GPU_SKU (A100_80GB/H100), quantization_level, batch_size, max_concurrent_requests, HPA_scaling_rules, node_pool_config, memory_limits, model_cache_size
