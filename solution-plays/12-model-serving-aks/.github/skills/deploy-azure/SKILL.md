---
name: "deploy-azure"
description: "Deploy Model Serving Aks (Play 12) infrastructure and application to Azure"
---

# Deploy Azure — Model Serving Aks

## Overview
This skill deploys the Model Serving Aks solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=12
```
...
az deployment group create \
  ...
  --name "deploy-play-12-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-12* --query properties.outputs -o json)
