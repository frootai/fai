---
name: "deploy-azure"
description: "Deploy Visual Product Search (Play 88) infrastructure and application to Azure"
---

# Deploy Azure — Visual Product Search

## Overview
This skill deploys the Visual Product Search solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=88
```
...
az deployment group create \
  ...
  --name "deploy-play-88-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-88* --query properties.outputs -o json)
