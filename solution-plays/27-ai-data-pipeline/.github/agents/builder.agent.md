...
- **Azure Data Factory** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: LLM-Augmented ETL
ETL with LLM augmentation, data classification, PII detection and redaction, schema inference, quality scoring, enrichment pipelines, batch vs streaming, data lineage tracking
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "27-ai-data-pipeline-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for AI Data Pipeline
classification_prompts, pii_rules (detect/redact/mask), quality_thresholds (0.8-0.95), batch_size (100-10000), enrichment_model (gpt-4o-mini for cost), retry_policy, parallelism_degree
