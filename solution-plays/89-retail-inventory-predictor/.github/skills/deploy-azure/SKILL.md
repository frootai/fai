---
name: "deploy-azure"
description: "Deploy Retail Inventory Predictor (Play 89) infrastructure and application to Azure"
---

# Deploy Azure — Retail Inventory Predictor

## Overview
This skill deploys the Retail Inventory Predictor solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=89
```
...
az deployment group create \
  ...
  --name "deploy-play-89-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-89* --query properties.outputs -o json)
