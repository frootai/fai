---
name: "deploy-azure"
description: "Deploy Climate Risk Assessor (Play 72) infrastructure and application to Azure"
---

# Deploy Azure — Climate Risk Assessor

## Overview
This skill deploys the Climate Risk Assessor solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=72
```
...
az deployment group create \
  ...
  --name "deploy-play-72-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-72* --query properties.outputs -o json)
