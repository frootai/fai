---
name: "deploy-azure"
description: "Deploy Anomaly Detection (Play 20) infrastructure and application to Azure"
---

# Deploy Azure — Anomaly Detection

## Overview
This skill deploys the Anomaly Detection solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=20
```
...
az deployment group create \
  ...
  --name "deploy-play-20-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-20* --query properties.outputs -o json)
