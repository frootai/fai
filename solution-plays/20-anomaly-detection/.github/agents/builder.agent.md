...
real-time event streaming, statistical anomaly scoring (z-score/IQR/isolation forest), LLM-augmented pattern explanation, alerting with severity classification, time-window analysis, baseline learning, multi-variate anomaly detection, sub-second processing latency
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "20-anomaly-detection-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Anomaly Detection
detection_window_minutes (1-60), anomaly_threshold_z_score (2.0-3.5), alert_severity_rules, baseline_learning_period_hours (24-168), event_batch_size, results_retention_days, signalr_connection_limit
