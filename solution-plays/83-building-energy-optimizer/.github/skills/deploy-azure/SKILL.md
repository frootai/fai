---
name: "deploy-azure"
description: "Deploy Building Energy Optimizer (Play 83) infrastructure and application to Azure"
---

# Deploy Azure — Building Energy Optimizer

## Overview
This skill deploys the Building Energy Optimizer solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=83
```
...
az deployment group create \
  ...
  --name "deploy-play-83-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-83* --query properties.outputs -o json)
