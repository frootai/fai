---
name: "deploy-azure"
description: "Deploy Responsible Ai Dashboard (Play 60) infrastructure and application to Azure"
---

# Deploy Azure — Responsible Ai Dashboard

## Overview
This skill deploys the Responsible Ai Dashboard solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=60
```
...
az deployment group create \
  ...
  --name "deploy-play-60-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-60* --query properties.outputs -o json)
