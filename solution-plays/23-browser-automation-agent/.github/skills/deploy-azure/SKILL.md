---
name: "deploy-azure"
description: "Deploy Browser Automation Agent (Play 23) infrastructure and application to Azure"
---

# Deploy Azure — Browser Automation Agent

## Overview
This skill deploys the Browser Automation Agent solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=23
```
...
az deployment group create \
  ...
  --name "deploy-play-23-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-23* --query properties.outputs -o json)
