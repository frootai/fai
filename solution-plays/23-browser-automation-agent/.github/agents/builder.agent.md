...
- **Playwright MCP** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: AI Web Task Execution
Playwright browser automation, GPT-4o Vision for page understanding, form filling, navigation, data extraction, screenshot analysis, cookie/session management, headless Chrome
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "23-browser-automation-agent-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Browser Automation
domain_allowlist, vision_prompt_templates, action_timeout_ms (5000-30000), retry_config, max_steps_per_task, screenshot_resolution, viewport_size
