---
name: "deploy-azure"
description: "Deploy Ai Api Gateway V2 (Play 52) infrastructure and application to Azure"
---

# Deploy Azure — Ai Api Gateway V2

## Overview
This skill deploys the Ai Api Gateway V2 solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=52
```
...
az deployment group create \
  ...
  --name "deploy-play-52-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-52* --query properties.outputs -o json)
