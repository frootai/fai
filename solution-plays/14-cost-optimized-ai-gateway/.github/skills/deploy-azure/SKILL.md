---
name: "deploy-azure"
description: "Deploy Cost Optimized Ai Gateway (Play 14) infrastructure and application to Azure"
---

# Deploy Azure — Cost Optimized Ai Gateway

## Overview
This skill deploys the Cost Optimized Ai Gateway solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=14
```
...
az deployment group create \
  ...
  --name "deploy-play-14-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-14* --query properties.outputs -o json)
