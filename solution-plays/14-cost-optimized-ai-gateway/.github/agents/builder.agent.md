...
- **Azure OpenAI (multi-region)** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: API Gateway FinOps
semantic caching, smart model routing (gpt-4o vs gpt-4o-mini by complexity), token budget enforcement per user/team, rate limiting, usage analytics dashboards, multi-region load balancing, fallback chains, cost attribution
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "14-cost-optimized-ai-gateway-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Cost-Optimized AI Gateway
cache_ttl_seconds (300-3600), token_budget_per_user_per_day, rate_limit_rpm, fallback_chain_order, routing_threshold (cost vs quality), semantic_similarity_threshold (0.92-0.98), model_selection_rules
