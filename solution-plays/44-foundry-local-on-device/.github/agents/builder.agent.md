...
### Architecture Pattern: Air-Gapped Local Inference
air-gapped LLM inference for data-sovereign environments, local model serving, cloud fallback on quality/latency threshold, encrypted model storage, OTA model updates via IoT Hub, device attestation, offline queue
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "44-foundry-local-on-device-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Foundry Local On-Device
local_model_config (phi-4-mini/phi-3.5), fallback_rules (latency_ms/quality_score), sync_schedule, max_offline_hours, encrypted_storage_enabled, device_attestation_interval, queue_max_size
