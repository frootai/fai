---
name: "deploy-azure"
description: "Deploy Foundry Local On Device (Play 44) infrastructure and application to Azure"
---

# Deploy Azure — Foundry Local On Device

## Overview
This skill deploys the Foundry Local On Device solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=44
```
...
az deployment group create \
  ...
  --name "deploy-play-44-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-44* --query properties.outputs -o json)
