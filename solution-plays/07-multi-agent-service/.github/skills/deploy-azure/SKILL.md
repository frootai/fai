---
name: "deploy-azure"
description: "Deploy Multi Agent Service (Play 07) infrastructure and application to Azure"
---

# Deploy Azure — Multi Agent Service

## Overview
This skill deploys the Multi Agent Service solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=07
```
...
az deployment group create \
  ...
  --name "deploy-play-07-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-07* --query properties.outputs -o json)
