...
- **Dapr** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: Supervisor-Specialist
supervisor agent, specialist delegation, shared state, handoff protocol, tool routing, loop prevention, memory management
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "07-multi-agent-service-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Multi-Agent Service
supervisor_routing, tool_schemas, agent_memory, max_iterations=10, delegation_rules, conflict_policy
