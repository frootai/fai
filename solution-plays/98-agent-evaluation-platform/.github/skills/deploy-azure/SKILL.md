---
name: "deploy-azure"
description: "Deploy Agent Evaluation Platform (Play 98) infrastructure and application to Azure"
---

# Deploy Azure — Agent Evaluation Platform

## Overview
This skill deploys the Agent Evaluation Platform solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=98
```
...
az deployment group create \
  ...
  --name "deploy-play-98-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-98* --query properties.outputs -o json)
