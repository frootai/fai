...
- **Azure VNet** — use latest SDK, Managed Identity auth, private endpoints where available
...
- **Azure RBAC** — use latest SDK, Managed Identity auth, private endpoints where available
...
- **GPU Quota** — use latest SDK, Managed Identity auth, private endpoints where available

### Architecture Pattern: Infrastructure
networking, identity, private endpoints, managed identity, hub-spoke topology, GPU quota management
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "02-ai-landing-zone-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for AI Landing Zone
VNet CIDR, subnet configuration, service SKUs, GPU quota, region selection, NSG rules
