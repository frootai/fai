---
name: "deploy-azure"
description: "Deploy Low Code Ai Builder (Play 31) infrastructure and application to Azure"
---

# Deploy Azure — Low Code Ai Builder

## Overview
This skill deploys the Low Code Ai Builder solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=31
```
...
az deployment group create \
  ...
  --name "deploy-play-31-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-31* --query properties.outputs -o json)
