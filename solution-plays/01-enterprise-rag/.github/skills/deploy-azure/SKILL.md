---
name: "deploy-azure"
description: "Deploy Enterprise Rag (Play 01) infrastructure and application to Azure"
---

# Deploy Azure — Enterprise Rag

## Overview
This skill deploys the Enterprise Rag solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=01
```
...
az deployment group create \
  ...
  --name "deploy-play-01-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-01* --query properties.outputs -o json)
