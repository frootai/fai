---
name: "deploy-azure"
description: "Deploy Citizen Services Chatbot (Play 84) infrastructure and application to Azure"
---

# Deploy Azure — Citizen Services Chatbot

## Overview
This skill deploys the Citizen Services Chatbot solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=84
```
...
az deployment group create \
  ...
  --name "deploy-play-84-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-84* --query properties.outputs -o json)
