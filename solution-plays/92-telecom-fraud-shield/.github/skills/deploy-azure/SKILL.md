---
name: "deploy-azure"
description: "Deploy Telecom Fraud Shield (Play 92) infrastructure and application to Azure"
---

# Deploy Azure — Telecom Fraud Shield

## Overview
This skill deploys the Telecom Fraud Shield solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=92
```
...
az deployment group create \
  ...
  --name "deploy-play-92-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-92* --query properties.outputs -o json)
