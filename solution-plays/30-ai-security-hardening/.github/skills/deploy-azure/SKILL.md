---
name: "deploy-azure"
description: "Deploy Ai Security Hardening (Play 30) infrastructure and application to Azure"
---

# Deploy Azure — Ai Security Hardening

## Overview
This skill deploys the Ai Security Hardening solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=30
```
...
az deployment group create \
  ...
  --name "deploy-play-30-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-30* --query properties.outputs -o json)
