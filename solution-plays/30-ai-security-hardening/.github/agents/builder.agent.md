...
- **Azure AI Content Safety** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: LLM Security Defense
prompt injection detection (direct/indirect), jailbreak defense, OWASP LLM Top 10, red teaming automation, PII leakage prevention, model endpoint protection, safety scorecards, EU AI Act compliance
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "30-ai-security-hardening-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for AI Security Hardening
injection_detection_sensitivity (low/medium/high), jailbreak_patterns, blocklist_terms, severity_thresholds (0-4), red_team_scenarios_count, pii_categories, compliance_framework (EU_AI_Act/NIST/ISO)
