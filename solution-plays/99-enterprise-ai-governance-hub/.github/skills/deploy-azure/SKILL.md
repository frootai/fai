---
name: "deploy-azure"
description: "Deploy Enterprise Ai Governance Hub (Play 99) infrastructure and application to Azure"
---

# Deploy Azure — Enterprise Ai Governance Hub

## Overview
This skill deploys the Enterprise Ai Governance Hub solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=99
```
...
az deployment group create \
  ...
  --name "deploy-play-99-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-99* --query properties.outputs -o json)
