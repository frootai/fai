---
name: "deploy-azure"
description: "Deploy Federated Learning Pipeline (Play 62) infrastructure and application to Azure"
---

# Deploy Azure — Federated Learning Pipeline

## Overview
This skill deploys the Federated Learning Pipeline solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=62
```
...
az deployment group create \
  ...
  --name "deploy-play-62-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-62* --query properties.outputs -o json)
