# AI-Powered Code Review Workflow — Federated Learning Pipeline

> Layer 3 — Agentic Workflow. Runs automated review on PRs targeting this play.

## Trigger
On pull request to `main` when files in `solution-plays/62-federated-learning-pipeline/` are modified.
...
```yaml
name: AI Review — Federated Learning Pipeline
on:
  pull_request:
    paths: ['solution-plays/62-federated-learning-pipeline/**']
  ...
  review:
    ...
    steps:
      ...
      - name: Check JSON validity
        run: |
          for f in $(find solution-plays/62-federated-learning-pipeline -name "*.json" -type f); do
            python3 -c "import json; json.load(open('$f'))" || echo "INVALID: $f"
          done

      - name: Check for secrets
        run: |
          if grep -rl "sk-[a-zA-Z0-9]\{20,\}" solution-plays/62-federated-learning-pipeline/ 2>/dev/null; then
      ...
      - name: Check guardrails
        run: |
          python3 -c "
          import json
          g = json.load(open('solution-plays/62-federated-learning-pipeline/config/guardrails.json'))
