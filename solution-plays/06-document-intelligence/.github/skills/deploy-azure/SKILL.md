---
name: "deploy-azure"
description: "Deploy Document Intelligence (Play 06) infrastructure and application to Azure"
---

# Deploy Azure — Document Intelligence

## Overview
This skill deploys the Document Intelligence solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=06
```
...
az deployment group create \
  ...
  --name "deploy-play-06-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-06* --query properties.outputs -o json)
