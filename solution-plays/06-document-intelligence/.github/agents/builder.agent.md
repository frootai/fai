...
### Architecture Pattern: OCR+LLM Extraction
OCR, form extraction, table recognition, handwriting, multi-page PDF, structured JSON output, PII masking
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "06-document-intelligence-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Document Intelligence
extraction_prompts, confidence_thresholds, field_schemas, document_classification_rules, page_ranges
