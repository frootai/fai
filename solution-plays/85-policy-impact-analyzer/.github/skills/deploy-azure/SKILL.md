---
name: "deploy-azure"
description: "Deploy Policy Impact Analyzer (Play 85) infrastructure and application to Azure"
---

# Deploy Azure — Policy Impact Analyzer

## Overview
This skill deploys the Policy Impact Analyzer solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=85
```
...
az deployment group create \
  ...
  --name "deploy-play-85-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-85* --query properties.outputs -o json)
