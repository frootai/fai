---
name: "deploy-azure"
description: "Deploy Customer Churn Predictor (Play 91) infrastructure and application to Azure"
---

# Deploy Azure — Customer Churn Predictor

## Overview
This skill deploys the Customer Churn Predictor solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=91
```
...
az deployment group create \
  ...
  --name "deploy-play-91-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-91* --query properties.outputs -o json)
