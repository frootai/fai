...
### Architecture Pattern: Persistent Agent Memory
short-term memory (Redis), long-term memory (Cosmos DB), episodic memory (AI Search vectors), cross-session learning, memory consolidation, forgetting policies, semantic recall
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "25-conversation-memory-layer-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Conversation Memory
memory_tiers (short/long/episodic), retention_policy_days, recall_strategy (semantic/temporal/hybrid), embedding_model, max_memory_items, consolidation_schedule, redis_ttl_seconds
