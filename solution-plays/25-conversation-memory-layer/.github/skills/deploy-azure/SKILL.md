---
name: "deploy-azure"
description: "Deploy Conversation Memory Layer (Play 25) infrastructure and application to Azure"
---

# Deploy Azure — Conversation Memory Layer

## Overview
This skill deploys the Conversation Memory Layer solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=25
```
...
az deployment group create \
  ...
  --name "deploy-play-25-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-25* --query properties.outputs -o json)
