---
name: "deploy-azure"
description: "Deploy Copilot Teams Extension (Play 16) infrastructure and application to Azure"
---

# Deploy Azure — Copilot Teams Extension

## Overview
This skill deploys the Copilot Teams Extension solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=16
```
...
az deployment group create \
  ...
  --name "deploy-play-16-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-16* --query properties.outputs -o json)
