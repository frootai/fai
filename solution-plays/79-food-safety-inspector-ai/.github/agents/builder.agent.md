...
- **Azure Document Intelligence** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: HACCP Compliance
HACCP plan compliance verification, real-time temperature monitoring (cold chain), contamination detection from sensor data, farm-to-fork traceability, pathogen risk scoring, recall management, audit report generation, regulatory submission preparation
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "79-food-safety-inspector-ai-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Food Safety Inspector AI
temperature_alert_thresholds_celsius (per_product_type), pathogen_models (salmonella/listeria/ecoli), audit_retention_years (5), traceability_depth (farm/processor/distributor/retailer), recall_trigger_rules, haccp_critical_control_points
