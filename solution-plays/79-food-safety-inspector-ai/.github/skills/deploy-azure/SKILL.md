---
name: "deploy-azure"
description: "Deploy Food Safety Inspector Ai (Play 79) infrastructure and application to Azure"
---

# Deploy Azure — Food Safety Inspector Ai

## Overview
This skill deploys the Food Safety Inspector Ai solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=79
```
...
az deployment group create \
  ...
  --name "deploy-play-79-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-79* --query properties.outputs -o json)
