---
name: "deploy-azure"
description: "Deploy Ai Customer Support V2 (Play 54) infrastructure and application to Azure"
---

# Deploy Azure — Ai Customer Support V2

## Overview
This skill deploys the Ai Customer Support V2 solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=54
```
...
az deployment group create \
  ...
  --name "deploy-play-54-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-54* --query properties.outputs -o json)
