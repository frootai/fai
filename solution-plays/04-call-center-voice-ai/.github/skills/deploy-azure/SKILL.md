---
name: "deploy-azure"
description: "Deploy Call Center Voice Ai (Play 04) infrastructure and application to Azure"
---

# Deploy Azure — Call Center Voice Ai

## Overview
This skill deploys the Call Center Voice Ai solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=04
```
...
az deployment group create \
  ...
  --name "deploy-play-04-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-04* --query properties.outputs -o json)
