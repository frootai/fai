...
### Architecture Pattern: Voice Pipeline
speech-to-text, text-to-speech, real-time streaming, intent recognition, call routing, escalation, PII redaction
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "04-call-center-voice-ai-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Call Center Voice AI
speech_language, speech_speed, grounding_prompts, fallback_chains, latency_targets, audio_encoding
