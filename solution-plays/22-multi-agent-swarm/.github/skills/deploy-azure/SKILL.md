---
name: "deploy-azure"
description: "Deploy Multi Agent Swarm (Play 22) infrastructure and application to Azure"
---

# Deploy Azure — Multi Agent Swarm

## Overview
This skill deploys the Multi Agent Swarm solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=22
```
...
az deployment group create \
  ...
  --name "deploy-play-22-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-22* --query properties.outputs -o json)
