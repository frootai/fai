# AI-Powered Deployment Workflow — Voice Ai Agent

> Layer 3 — Agentic Workflow. Compiles to GitHub Actions for automated Azure deployment.

## Trigger
On push to `main` branch after PR merge, when files in `solution-plays/33-voice-ai-agent/infra/` are modified.
...
```yaml
name: AI Deploy — Voice Ai Agent
on:
  push:
    branches: [main]
    paths: ['solution-plays/33-voice-ai-agent/infra/**']
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Bicep
        run: az bicep build --file solution-plays/33-voice-ai-agent/infra/main.bicep
      - name: Validate Configs
        run: |
          for f in solution-plays/33-voice-ai-agent/config/*.json; do
      ...
      - name: Deploy to Staging
        run: |
          az deployment group create \
            --resource-group rg-voice-ai-agent-staging \
            --template-file solution-plays/33-voice-ai-agent/infra/main.bicep \
            --parameters solution-plays/33-voice-ai-agent/infra/parameters.json
  ...
  evaluate:
    ...
    steps:
      ...
      - name: Run Evaluation
        run: python solution-plays/33-voice-ai-agent/evaluation/eval.py
        env:
  ...
  deploy-production:
    ...
    steps:
      ...
      - name: Deploy to Production
        run: |
          az deployment group create \
            --resource-group rg-voice-ai-agent \
            --template-file solution-plays/33-voice-ai-agent/infra/main.bicep \
            --parameters solution-plays/33-voice-ai-agent/infra/parameters.json \
