---
name: "deploy-azure"
description: "Deploy Voice Ai Agent (Play 33) infrastructure and application to Azure"
---

# Deploy Azure — Voice Ai Agent

## Overview
This skill deploys the Voice Ai Agent solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=33
```
...
az deployment group create \
  ...
  --name "deploy-play-33-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-33* --query properties.outputs -o json)
