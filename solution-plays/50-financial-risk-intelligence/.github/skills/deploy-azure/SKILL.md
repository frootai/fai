---
name: "deploy-azure"
description: "Deploy Financial Risk Intelligence (Play 50) infrastructure and application to Azure"
---

# Deploy Azure — Financial Risk Intelligence

## Overview
This skill deploys the Financial Risk Intelligence solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=50
```
...
az deployment group create \
  ...
  --name "deploy-play-50-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-50* --query properties.outputs -o json)
