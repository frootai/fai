---
name: "deploy-azure"
description: "Deploy Knowledge Graph Rag (Play 28) infrastructure and application to Azure"
---

# Deploy Azure — Knowledge Graph Rag

## Overview
This skill deploys the Knowledge Graph Rag solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=28
```
...
az deployment group create \
  ...
  --name "deploy-play-28-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-28* --query properties.outputs -o json)
