...
- **Azure Cosmos DB (Gremlin API)** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: Graph-Enhanced Retrieval
entity extraction (NER), relationship mapping, graph traversal for context, Gremlin queries, hybrid graph+vector retrieval, explainable answers with provenance chains, ontology management
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "28-knowledge-graph-rag-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Knowledge Graph RAG
graph_depth (1-4 hops), entity_types, relationship_weights, answer_style (concise/detailed/explainable), entity_confidence_threshold, max_graph_nodes_per_query, embedding_model
