...
- **Azure OpenAI (GPT-4o Vision)** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: Vision+Text Extraction
multi-modal document processing, image+text+table extraction, GPT-4o vision analysis, handwriting OCR, chart/diagram interpretation, structured JSON output, batch processing pipeline, PII masking
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "15-multi-modal-docproc-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Multi-Modal DocProc
image_resolution (low/medium/high), extraction_schemas, confidence_thresholds, page_ranges, batch_size, vision_prompt_templates, output_format (JSON/CSV/Markdown)
