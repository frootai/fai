---
name: "deploy-azure"
description: "Deploy Multi Modal Docproc (Play 15) infrastructure and application to Azure"
---

# Deploy Azure — Multi Modal Docproc

## Overview
This skill deploys the Multi Modal Docproc solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=15
```
...
az deployment group create \
  ...
  --name "deploy-play-15-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-15* --query properties.outputs -o json)
