---
name: "deploy-azure"
description: "Deploy Edge Ai Deployment (Play 34) infrastructure and application to Azure"
---

# Deploy Azure — Edge Ai Deployment

## Overview
This skill deploys the Edge Ai Deployment solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=34
```
...
az deployment group create \
  ...
  --name "deploy-play-34-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-34* --query properties.outputs -o json)
