# AI-Powered Deployment Workflow — Edge Ai Deployment

> Layer 3 — Agentic Workflow. Compiles to GitHub Actions for automated Azure deployment.

## Trigger
On push to `main` branch after PR merge, when files in `solution-plays/34-edge-ai-deployment/infra/` are modified.
...
```yaml
name: AI Deploy — Edge Ai Deployment
on:
  push:
    branches: [main]
    paths: ['solution-plays/34-edge-ai-deployment/infra/**']
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Bicep
        run: az bicep build --file solution-plays/34-edge-ai-deployment/infra/main.bicep
      - name: Validate Configs
        run: |
          for f in solution-plays/34-edge-ai-deployment/config/*.json; do
      ...
      - name: Deploy to Staging
        run: |
          az deployment group create \
            --resource-group rg-edge-ai-deployment-staging \
            --template-file solution-plays/34-edge-ai-deployment/infra/main.bicep \
            --parameters solution-plays/34-edge-ai-deployment/infra/parameters.json
  ...
  evaluate:
    ...
    steps:
      ...
      - name: Run Evaluation
        run: python solution-plays/34-edge-ai-deployment/evaluation/eval.py
        env:
  ...
  deploy-production:
    ...
    steps:
      ...
      - name: Deploy to Production
        run: |
          az deployment group create \
            --resource-group rg-edge-ai-deployment \
            --template-file solution-plays/34-edge-ai-deployment/infra/main.bicep \
            --parameters solution-plays/34-edge-ai-deployment/infra/parameters.json \
