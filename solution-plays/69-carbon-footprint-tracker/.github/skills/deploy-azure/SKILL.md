---
name: "deploy-azure"
description: "Deploy Carbon Footprint Tracker (Play 69) infrastructure and application to Azure"
---

# Deploy Azure — Carbon Footprint Tracker

## Overview
This skill deploys the Carbon Footprint Tracker solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=69
```
...
az deployment group create \
  ...
  --name "deploy-play-69-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-69* --query properties.outputs -o json)
