---
name: "deploy-azure"
description: "Deploy It Ticket Resolution (Play 05) infrastructure and application to Azure"
---

# Deploy Azure — It Ticket Resolution

## Overview
This skill deploys the It Ticket Resolution solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=05
```
...
az deployment group create \
  ...
  --name "deploy-play-05-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-05* --query properties.outputs -o json)
