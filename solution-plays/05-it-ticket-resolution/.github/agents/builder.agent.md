...
- **Azure Logic Apps** — use latest SDK, Managed Identity auth, private endpoints where available
...
- **Azure Service Bus** — use latest SDK, Managed Identity auth, private endpoints where available
- **ServiceNow MCP** — use latest SDK, Managed Identity auth, private endpoints where available

### Architecture Pattern: Event-Driven Automation
ticket classification, routing, auto-resolution, ITSM integration, ServiceNow, confidence scoring, escalation
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "05-it-ticket-resolution-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for IT Ticket Resolution
classification_prompts, routing_rules, confidence_thresholds, escalation_criteria, batch_size
