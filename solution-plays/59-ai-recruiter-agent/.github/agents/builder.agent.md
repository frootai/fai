...
### Architecture Pattern: Talent Intelligence
resume screening with bias detection (gender/age/ethnicity), job-candidate matching via embeddings, interview scheduling via Graph Calendar API, skills assessment generation, diversity analytics, candidate experience scoring
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "59-ai-recruiter-agent-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for AI Recruiter Agent
matching_weights (skills:0.4/experience:0.3/education:0.2/culture:0.1), bias_detection_rules (protected_categories), scheduling_config (timezone/availability), diversity_targets, candidate_score_threshold
