...
- **Azure AI Voice Live** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: Bidirectional Voice
sub-200ms bidirectional voice, WebSocket streaming, MCP tool calling during conversation, avatar rendering integration, voice activity detection (VAD), barge-in support, multi-language real-time switching, emotion detection
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "96-realtime-voice-agent-v2-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Real-Time Voice Agent v2
vad_mode (server/client/hybrid), latency_target_ms (<200), function_calling_timeout_ms (3000), avatar_rendering_enabled, barge_in_sensitivity, supported_languages_realtime, emotion_detection_enabled, websocket_keepalive_interval_ms
