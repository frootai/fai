---
name: "deploy-azure"
description: "Deploy Realtime Event Ai (Play 45) infrastructure and application to Azure"
---

# Deploy Azure — Realtime Event Ai

## Overview
This skill deploys the Realtime Event Ai solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=45
```
...
az deployment group create \
  ...
  --name "deploy-play-45-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-45* --query properties.outputs -o json)
