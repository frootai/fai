---
name: "deploy-azure"
description: "Deploy Ai Code Review Pipeline (Play 24) infrastructure and application to Azure"
---

# Deploy Azure — Ai Code Review Pipeline

## Overview
This skill deploys the Ai Code Review Pipeline solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=24
```
...
az deployment group create \
  ...
  --name "deploy-play-24-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-24* --query properties.outputs -o json)
