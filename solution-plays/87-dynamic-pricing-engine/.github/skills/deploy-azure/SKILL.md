---
name: "deploy-azure"
description: "Deploy Dynamic Pricing Engine (Play 87) infrastructure and application to Azure"
---

# Deploy Azure — Dynamic Pricing Engine

## Overview
This skill deploys the Dynamic Pricing Engine solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=87
```
...
az deployment group create \
  ...
  --name "deploy-play-87-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-87* --query properties.outputs -o json)
