---
name: "deploy-azure"
description: "Deploy Precision Agriculture Agent (Play 78) infrastructure and application to Azure"
---

# Deploy Azure — Precision Agriculture Agent

## Overview
This skill deploys the Precision Agriculture Agent solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=78
```
...
az deployment group create \
  ...
  --name "deploy-play-78-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-78* --query properties.outputs -o json)
