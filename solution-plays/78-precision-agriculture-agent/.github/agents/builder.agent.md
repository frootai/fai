...
- **Azure Digital Twins** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: Smart Farming
satellite imagery analysis for crop health (NDVI), IoT soil sensor fusion (moisture/pH/nitrogen), irrigation scheduling optimization, fertilization recommendations, yield prediction models, pest/disease early detection, weather integration
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "78-precision-agriculture-agent-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Precision Agriculture Agent
sensor_sampling_interval_minutes (5-60), imagery_frequency_days (1-7), irrigation_threshold_soil_moisture (30-60%), ndvi_health_threshold, pest_detection_confidence (0.8), weather_forecast_source, yield_prediction_horizon_weeks
