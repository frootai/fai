...
- **Azure Advisor** — use latest SDK, Managed Identity auth, private endpoints where available
- **Azure Cost Management** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: FinOps Automation
auto right-sizing recommendations, GPU utilization analysis, idle resource detection, FinOps reports with cost attribution, reservation recommendations, spot instance optimization, scaling rule tuning
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "66-ai-infrastructure-optimizer-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for AI Infrastructure Optimizer
optimization_rules (cpu_threshold:70%/memory:80%), alert_thresholds, reporting_schedule (daily/weekly), idle_detection_hours (72), gpu_utilization_target (80%), reservation_analysis_lookback_days (30)
