---
name: "deploy-azure"
description: "Deploy Ai Infrastructure Optimizer (Play 66) infrastructure and application to Azure"
---

# Deploy Azure — Ai Infrastructure Optimizer

## Overview
This skill deploys the Ai Infrastructure Optimizer solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=66
```
...
az deployment group create \
  ...
  --name "deploy-play-66-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-66* --query properties.outputs -o json)
