---
name: "deploy-azure"
description: "Deploy Digital Twin Agent (Play 58) infrastructure and application to Azure"
---

# Deploy Azure — Digital Twin Agent

## Overview
This skill deploys the Digital Twin Agent solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=58
```
...
az deployment group create \
  ...
  --name "deploy-play-58-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-58* --query properties.outputs -o json)
