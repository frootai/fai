---
name: "deploy-azure"
description: "Deploy Supply Chain Ai (Play 55) infrastructure and application to Azure"
---

# Deploy Azure — Supply Chain Ai

## Overview
This skill deploys the Supply Chain Ai solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=55
```
...
az deployment group create \
  ...
  --name "deploy-play-55-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-55* --query properties.outputs -o json)
