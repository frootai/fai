# AI-Powered Deployment Workflow — Agentic Rag

> Layer 3 — Agentic Workflow. Compiles to GitHub Actions for automated Azure deployment.

## Trigger
On push to `main` branch after PR merge, when files in `solution-plays/21-agentic-rag/infra/` are modified.
...
```yaml
name: AI Deploy — Agentic Rag
on:
  push:
    branches: [main]
    paths: ['solution-plays/21-agentic-rag/infra/**']
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Bicep
        run: az bicep build --file solution-plays/21-agentic-rag/infra/main.bicep
      - name: Validate Configs
        run: |
          for f in solution-plays/21-agentic-rag/config/*.json; do
      ...
      - name: Deploy to Staging
        run: |
          az deployment group create \
            --resource-group rg-agentic-rag-staging \
            --template-file solution-plays/21-agentic-rag/infra/main.bicep \
            --parameters solution-plays/21-agentic-rag/infra/parameters.json
  ...
  evaluate:
    ...
    steps:
      ...
      - name: Run Evaluation
        run: python solution-plays/21-agentic-rag/evaluation/eval.py
        env:
