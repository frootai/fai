---
name: "deploy-azure"
description: "Deploy Agentic Rag (Play 21) infrastructure and application to Azure"
---

# Deploy Azure — Agentic Rag

## Overview
This skill deploys the Agentic Rag solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=21
```
...
az deployment group create \
  ...
  --name "deploy-play-21-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-21* --query properties.outputs -o json)
