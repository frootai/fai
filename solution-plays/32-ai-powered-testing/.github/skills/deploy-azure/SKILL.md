---
name: "deploy-azure"
description: "Deploy Ai Powered Testing (Play 32) infrastructure and application to Azure"
---

# Deploy Azure — Ai Powered Testing

## Overview
This skill deploys the Ai Powered Testing solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=32
```
...
az deployment group create \
  ...
  --name "deploy-play-32-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-32* --query properties.outputs -o json)
