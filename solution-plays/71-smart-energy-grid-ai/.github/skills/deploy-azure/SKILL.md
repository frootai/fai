---
name: "deploy-azure"
description: "Deploy Smart Energy Grid Ai (Play 71) infrastructure and application to Azure"
---

# Deploy Azure — Smart Energy Grid Ai

## Overview
This skill deploys the Smart Energy Grid Ai solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=71
```
...
az deployment group create \
  ...
  --name "deploy-play-71-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-71* --query properties.outputs -o json)
