---
name: "deploy-azure"
description: "Deploy Predictive Maintenance Ai (Play 68) infrastructure and application to Azure"
---

# Deploy Azure — Predictive Maintenance Ai

## Overview
This skill deploys the Predictive Maintenance Ai solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=68
```
...
az deployment group create \
  ...
  --name "deploy-play-68-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-68* --query properties.outputs -o json)
