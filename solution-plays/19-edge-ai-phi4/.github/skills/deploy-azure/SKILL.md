---
name: "deploy-azure"
description: "Deploy Edge Ai Phi4 (Play 19) infrastructure and application to Azure"
---

# Deploy Azure — Edge Ai Phi4

## Overview
This skill deploys the Edge Ai Phi4 solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=19
```
...
az deployment group create \
  ...
  --name "deploy-play-19-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-19* --query properties.outputs -o json)
