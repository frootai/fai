...
- **Azure Container Instances** — use latest SDK, Managed Identity auth, private endpoints where available
- **ONNX Runtime** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: On-Device Inference
Phi-4 SLM on-device deployment, ONNX quantization (INT4/INT8/FP16), offline inference capability, cloud-edge sync for model updates OTA, fallback to cloud inference on quality/latency threshold, battery and memory optimization, edge device fleet management
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "19-edge-ai-phi4-request", properties: { correlationId } });
    // ... implementation
...
### Cost Optimization
- Use `phi-4-mini` with `max_tokens` from config (never unlimited)
...
## Tuning Parameters for Edge AI Phi-4
quantization_level (INT4/INT8/FP16), sync_schedule (hourly/daily), fallback_threshold_latency_ms, fallback_threshold_quality, max_memory_mb, batch_inference_enabled, model_cache_size_mb, device_telemetry_interval
