---
name: "deploy-azure"
description: "Deploy Construction Safety Ai (Play 82) infrastructure and application to Azure"
---

# Deploy Azure — Construction Safety Ai

## Overview
This skill deploys the Construction Safety Ai solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=82
```
...
az deployment group create \
  ...
  --name "deploy-play-82-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-82* --query properties.outputs -o json)
