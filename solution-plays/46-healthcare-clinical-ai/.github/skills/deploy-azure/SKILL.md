---
name: "deploy-azure"
description: "Deploy Healthcare Clinical Ai (Play 46) infrastructure and application to Azure"
---

# Deploy Azure — Healthcare Clinical Ai

## Overview
This skill deploys the Healthcare Clinical Ai solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=46
```
...
az deployment group create \
  ...
  --name "deploy-play-46-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-46* --query properties.outputs -o json)
