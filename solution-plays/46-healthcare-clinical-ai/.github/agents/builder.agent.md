...
- **Azure Health Data Services (FHIR)** — use latest SDK, Managed Identity auth, private endpoints where available
...
### Architecture Pattern: HIPAA-Compliant Clinical Agent
HIPAA compliance, medical coding (ICD-10/CPT), clinical note summarization, drug interaction checking via FHIR, patient data de-identification, audit trails, human-in-the-loop approval for clinical decisions
...
async function processRequest(req: HttpRequest): Promise<HttpResponseInit> {
  ...
  try {
    telemetry.trackEvent({ name: "46-healthcare-clinical-ai-request", properties: { correlationId } });
    // ... implementation
...
## Tuning Parameters for Healthcare Clinical AI
medical_accuracy_threshold (0.95), audit_retention_days (2555 for HIPAA), approval_rules (always_human_for_diagnosis), fhir_version (R4), pii_categories (PHI), confidence_for_auto_approve (0.98)
