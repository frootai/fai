---
name: "deploy-azure"
description: "Deploy Accessibility Learning Agent (Play 76) infrastructure and application to Azure"
---

# Deploy Azure — Accessibility Learning Agent

## Overview
This skill deploys the Accessibility Learning Agent solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=76
```
...
az deployment group create \
  ...
  --name "deploy-play-76-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-76* --query properties.outputs -o json)
