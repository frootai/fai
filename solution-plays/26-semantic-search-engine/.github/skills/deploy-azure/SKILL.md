---
name: "deploy-azure"
description: "Deploy Semantic Search Engine (Play 26) infrastructure and application to Azure"
---

# Deploy Azure — Semantic Search Engine

## Overview
This skill deploys the Semantic Search Engine solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=26
```
...
az deployment group create \
  ...
  --name "deploy-play-26-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-26* --query properties.outputs -o json)
