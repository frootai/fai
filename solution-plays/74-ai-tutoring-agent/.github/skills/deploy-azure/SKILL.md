---
name: "deploy-azure"
description: "Deploy Ai Tutoring Agent (Play 74) infrastructure and application to Azure"
---

# Deploy Azure — Ai Tutoring Agent

## Overview
This skill deploys the Ai Tutoring Agent solution play to Azure using Bicep infrastructure-as-code and Azure Developer CLI.
...
az group create --name $RESOURCE_GROUP --location $LOCATION --tags project=frootai environment=${ENVIRONMENT:-dev} play=74
```
...
az deployment group create \
  ...
  --name "deploy-play-74-$(date +%Y%m%d%H%M)"
```

## Step 4: Configure Application
```bash
# Get deployment outputs
OUTPUTS=$(az deployment group show --resource-group $RESOURCE_GROUP --name deploy-play-74* --query properties.outputs -o json)
