...
> Java MCP Development — build MCP servers in Java with Spring Boot, reactive streams, Maven/Gradle builds, and enterprise middleware integration.
...
```bash
npx frootai install java-mcp-development
```
...
| Instruction | `java-mcp-development` | Java mcp development standards |
| Instruction | `java-waf` | Java waf standards |
| Skill | `frootai-mcp-java-scaffold` | Mcp java scaffold capability |
| Skill | `frootai-mcp-java-scaffold` | Mcp java scaffold capability |
...
`java` `spring-boot` `mcp` `maven` `reactor` `model-context-protocol` `tool-server`
...
2. Edit files in `plugins/java-mcp-development/`
