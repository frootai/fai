...
| Field | Value |
|-------|-------|
| **Triggers** | `sessionStart`, `sessionEnd`, `userPromptSubmitted` |
...
| `event` | `sessionStart`, `sessionEnd`, or `userPromptSubmitted` |
...
| `duration_sec` | Session length in seconds (on `sessionEnd` only) |
| `level` | Always `info` |
...
| `LOG_RATE_LIMIT_SEC` | `1` | Min seconds between `userPromptSubmitted` entries |
...
Copy this folder to `.github/hooks/` or reference from a FrootAI plugin:

```bash
cp -r hooks/frootai-session-logger .github/hooks/
```
