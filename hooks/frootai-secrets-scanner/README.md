...
SCAN_MODE=block SCAN_SCOPE=staged bash .github/hooks/frootai-secrets-scanner/scan-secrets.sh
```
...
```yaml
- name: Secrets Scan
  run: bash .github/hooks/frootai-secrets-scanner/scan-secrets.sh
  env:
    SCAN_MODE: block
...
```bash
cp -r hooks/frootai-secrets-scanner .github/hooks/
```
