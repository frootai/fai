...
- name: WAF Compliance Check
  run: bash .github/hooks/frootai-waf-compliance/check-waf.sh
  env:
    WAF_MODE: block
    WAF_THRESHOLD: "70"
...
```yaml
- script: bash .github/hooks/frootai-waf-compliance/check-waf.sh
  env:
    WAF_MODE: block
    WAF_THRESHOLD: "70"
...
```bash
cp -r hooks/frootai-waf-compliance .github/hooks/
```
