...
STATE_DIR="${TMPDIR:-/tmp}/FAI-budget"
mkdir -p "$STATE_DIR"
...
main() {
  ...
  if [ "$prev_percent" -lt "$ALERT_PCT" ] && [ "$percent" -ge "$ALERT_PCT" ]; then
    echo "💰 FAI Budget Alert: ${percent}% used (${new_usage}/${BUDGET} tokens)"
  fi
  ...
  if [ "$grace_count" -le "$GRACE_CALLS" ]; then
    echo "💰 FAI Budget: grace call ${grace_count}/${GRACE_CALLS} (budget exceeded)"
    exit 0
  fi

  # Over budget and past grace period
  echo ""
  echo "💰 FAI Token Budget Enforcer"
  echo "─────────────────────────────────────────"
