...
function markdownToHtml(markdown, title) {
  // Extract mermaid blocks before passing to marked — they need special rendering
  /** @type {string[]} */
  const mermaidBlocks = [];
  const processed = markdown.replace(/`{2,3}\s*mermaid\s*\n([\s\S]*?)`{2,3}/g, (/** @type {string} */ _match, /** @type {string} */ code) => {
    mermaidBlocks.push(code.trim());
    return `<div class="mermaid" id="mermaid-placeholder-${mermaidBlocks.length - 1}"></div>`;
  });

  // Escape the markdown for safe embedding in a <script> tag
  const escapedMd = JSON.stringify(processed);
  // Mermaid block data for re-insertion after marked renders
...
<head>
  ...
  <style>
    ...
    th { font-weight: 600; background: #1a1a3e; }
...
<body>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      // Render markdown with marked.js (production-grade CommonMark parser)
      var md = ${escapedMd};
      var mermaidBlocks = ${mermaidData};
      ...
      // Re-insert mermaid diagram content into placeholder divs
      mermaidBlocks.forEach(function(code, i) {
        var el = document.getElementById('mermaid-placeholder-' + i);
        if (el) el.textContent = code;
      });
      // Initialize mermaid rendering
      if (typeof mermaid !== 'undefined') {
        mermaid.initialize({ startOnLoad: false, theme: 'dark', themeVariables: { primaryColor: '#1a1a2e', primaryTextColor: '#e0e0e0', primaryBorderColor: '#6366f1', lineColor: '#818cf8', background: 'transparent' } });
        mermaid.run();
      }
    });
  <\/script>
