---
name: frootai-contributors-report
  ...
  workflow_dispatch:
    inputs:
      ...
      end_date:
        ...
        required: false
      include_website:
        description: "Include frootai.dev repo stats"
        type: boolean
        required: false
        default: true
permissions:
...
```bash
PRS_OPENED=$(gh pr list --repo frootai/frootai --state all --search "created:$START..$END" \
  --json number 2>/dev/null | node -e "console.log(JSON.parse(require('fs').readFileSync('/dev/stdin','utf8')).length)" 2>/dev/null || echo "N/A")
PRS_MERGED=$(gh pr list --repo frootai/frootai --state merged --search "merged:$START..$END" \
  --json number 2>/dev/null | node -e "console.log(JSON.parse(require('fs').readFileSync('/dev/stdin','utf8')).length)" 2>/dev/null || echo "N/A")
...
```bash
ISSUES_OPENED=$(gh issue list --repo frootai/frootai --state all --search "created:$START..$END" \
  --json number 2>/dev/null | node -e "console.log(JSON.parse(require('fs').readFileSync('/dev/stdin','utf8')).length)" 2>/dev/null || echo "N/A")
ISSUES_CLOSED=$(gh issue list --repo frootai/frootai --state closed --search "closed:$START..$END" \
  --json number 2>/dev/null | node -e "console.log(JSON.parse(require('fs').readFileSync('/dev/stdin','utf8')).length)" 2>/dev/null || echo "N/A")
...
gh pr list --repo frootai/frootai --state merged --search "merged:$START..$END" \
...
```markdown
# 🍊 FrootAI Contributors Report — [Month Year]
...
## Thank You! 🧡
Every contribution moves FrootAI forward. Shout-outs to first-timers, streak holders, and consistent builders.
