---
name: frootai-primitive-pr-review
...
|------|---------|-----------|
| Agent | `agents/<name>.agent.md` | lowercase-hyphen, `frootai-` prefix |
...
| Hook | `hooks/<folder>/hooks.json` | folder lowercase-hyphen, `frootai-` prefix |
| Plugin | `plugins/<folder>/plugin.json` | folder lowercase-hyphen |
| Workflow | `workflows/<name>.md` | lowercase-hyphen, `frootai-` prefix |
...
for NEW_AGENT in $(echo "$CHANGED" | grep 'agents/.*\.agent\.md$'); do
  NEW_STEM=$(basename "$NEW_AGENT" .agent.md | sed 's/^frootai-//')
  for EXISTING in agents/*.agent.md; do
    [ "$EXISTING" = "$NEW_AGENT" ] && continue
    EXIST_STEM=$(basename "$EXISTING" .agent.md | sed 's/^frootai-//')
...
|------|------|--------|-------------|-----|------|--------|
| Agent | frootai-xyz.agent.md | ✅ | ✅ | 3/6 | 4KB | ✅ |
...
### ⚠️ Warnings
1. `agents/frootai-new-agent.agent.md` — WAF coverage 1/6
