...
` });
        ...
        for (const role of ["builder", "reviewer", "tuner"]) {
          ...
          files.push({
            path: `.github/agents/${role}.agent.md`, content: `---
...
` });
        ...
        // ── .github/instructions/ ───────────────────────────────
        files.push({
          path: `.github/instructions/${slug}-patterns.instructions.md`, content: `---
...
` });
        ...
        for (const [cmd, purpose] of [["deploy", "Deploy to Azure"], ["test", "Run test suite"], ["review", "Code review"], ["evaluate", "Run evaluation pipeline"]]) {
          files.push({
            path: `.github/prompts/${cmd}.prompt.md`, content: `---
...
` });
        ...
        for (const skill of ["deploy", "evaluate", "tune"]) {
          files.push({
            path: `.github/skills/${skill}-${slug}/SKILL.md`, content: `---
name: "${skill}-${slug}"
...
` });
        ...
        // ── .github/hooks/ ──────────────────────────────────────
        files.push({
          path: ".github/hooks/guardrails.json", content: JSON.stringify({
            version: 1,
            hooks: {
        ...
        // ── .vscode/ ────────────────────────────────────────────
        files.push({
          path: ".vscode/mcp.json", content: JSON.stringify({
            servers: { frootai: { type: "stdio", command: "npx", args: ["frootai-mcp@latest"] } }
          }, null, 2)
        });

        files.push({
          path: ".vscode/settings.json", content: JSON.stringify({
        ...
        // ── config/ (TuneKit) ───────────────────────────────────
        files.push({
          path: "config/openai.json", content: JSON.stringify({
        ...
        files.push({
          path: "config/guardrails.json", content: JSON.stringify({
        ...
        // ── evaluation/ ─────────────────────────────────────────
        files.push({
          path: "evaluation/eval.py", content: `"""
...
` });
        ...
        files.push({
          path: "evaluation/test-set.jsonl", content: `{"question": "What does ${name} do?", "ground_truth": "${desc}", "context": ""}
...
` });
        ...
        if (generateInfra) {
          files.push({
            path: "infra/main.bicep", content: `// ${name} — Azure Infrastructure
...
` });
          ...
          files.push({
            path: "infra/parameters.json", content: JSON.stringify({
        ...
        files.push({
          path: "spec/play-spec.json", content: JSON.stringify({
