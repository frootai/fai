---
name: frootai-deploy-23-browser-automation-agent
description: 'Deploys Play 23-browser-automation-agent to Azure with Bicep validation, what-if check, and post-deploy health verification.'
---

# Fai Deploy 23 Browser Automation Agent

Deploys Play 23-browser-automation-agent to Azure with Bicep validation, what-if check, and post-deploy health verification.

## Overview

This skill provides a structured, repeatable procedure for deploys play 23-browser-automation-agent to azure with bicep validation, what-if check, and post-deploy health verification.. It can be used standalone as a LEGO block or auto-wired inside solution plays via the FAI Protocol.
...
Perform the primary operation: deploys play 23-browser-automation-agent to azure with bicep validation, what-if check, and post-deploy health verification..
...
npx frootai skill run frootai-deploy-23-browser-automation-agent
```
...
{
  "primitives": {
    "skills": ["skills/frootai-deploy-23-browser-automation-agent/"]
  }
}
```
