...
|-------|---------|----------|---------------|
| Shadow | 0% live (mirror only) | 1 hour | No crashes, latency within 2x baseline |
| Canary | 5% | 30 min | p99 latency <500ms, error rate <0.5% |
| Expand | 25% | 2 hours | Groundedness ≥4.2, relevance ≥4.0 |
| Majority | 50% | 4 hours | All metrics stable, no regression >5% |
...
(( $(echo "$LATENCY_P99 > 500" | bc -l) )) && echo "FAIL: p99 latency ${LATENCY_P99}ms > 500ms" && FAIL=1
(( $(echo "$ERROR_RATE > 0.5" | bc -l) )) && echo "FAIL: error rate ${ERROR_RATE}% > 0.5%" && FAIL=1
